...
describe("TestModifyVariable", function () {
    it("should change x to 10000", async function () {
        ...
        const contract = await ModifyVariable.deploy(10, "Hi");

        // wait for contract to be deployed and validated!
        await contract.deployed();

        // modify x from 10 to 1337 via this function!
        await contract.modifyToTenThousand();
        // getter for state variable x
        const newX = await contract.x();
        assert.equal(newX.toNumber(), 10000);
    });

    it("should change randomString to 'Holy moly this is a different string!'", async function () {
        // this line creates an ethers ContractFactory abstraction: https://docs.ethers.org/v5/api/contract/contract-factory/
        const ModifyVariable = await ethers.getContractFactory("ModifyVariable");
        ...
        // getter for state variable x
        const newString = await contract.randomString();
        assert.equal(newString.toString(), "Holy moly this is a different string!");
    });
});